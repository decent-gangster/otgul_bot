...
from database.crud import update_request_status, get_requests_by_user
...
async def approve_request(
    ...
    async with AsyncSessionFactory() as session:
        ...
        await update_request_status(session, request_id, status="approved")
    ...
    await call.message.edit_text(
        call.message.text + f"\n\n✅ <b>Одобрено</b> администратором @{call.from_user.username or call.from_user.full_name}",
        parse_mode="HTML"
    )
...
async def reject_request_save(message: Message, state: FSMContext, bot: Bot):
    ...
    async with AsyncSessionFactory() as session:
        ...
        req, user = row
        await update_request_status(session, request_id, status="rejected", admin_comment=comment)
